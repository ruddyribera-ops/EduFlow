...
export default function BroadcastPage() {
  ...
  return (
    <div className="p-6 space-y-5 max-w-4xl">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">{t("broadcast.title")}</h1>
        <p className="text-sm text-slate-500 mt-1">{t("broadcast.subtitle")}</p>
      </div>

      <div className="bg-white border border-slate-200 rounded-lg p-5 space-y-4">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">{t("broadcast.message")}</label>
          <textarea
            value={message}
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-2">{t("broadcast.target")}</label>
          <div className="flex gap-4">
            <label className="flex items-center gap-2 text-sm">
              <input type="radio" checked={scope === "all"} onChange={() => setScope("all")} />
              {t("broadcast.allGuardians")}
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="radio" checked={scope === "students"} onChange={() => setScope("students")} />
              {t("broadcast.specificStudents")}
            </label>
          </div>
        </div>
        ...
        {scope === "students" && studentsRes && (
          <div className="border border-slate-200 rounded p-3 max-h-60 overflow-y-auto">
            <div className="text-xs font-medium text-slate-600 mb-2">
              {t("broadcast.selectStudents")} ({selected.length} {t("common.cancel").toLowerCase()})
            </div>
        ...
        {result && (
          <div className="text-sm bg-green-50 border border-green-200 rounded px-3 py-2 text-green-800">
            <div className="font-medium">{t("broadcast.success")}</div>
            <div className="mt-1 text-xs">
              {t("broadcast.recipients")}: {result.total} · {t("broadcast.emails")}: {result.email_sent} · {t("broadcast.sms")}: {result.sms_sent} · {t("broadcast.skipped")}: {result.skipped}
            </div>
          </div>
        )}
      ...
      {historyRes && historyRes.data.length > 0 && (
        <div className="bg-white border border-slate-200 rounded-lg p-5">
          <h2 className="font-semibold text-slate-900 mb-3">{t("broadcast.recent")}</h2>
            ...
            {historyRes.data.slice(0, 5).map((b) => (
              <div key={b.id} className="border-b border-slate-100 last:border-0 pb-2 last:pb-0">
                ...
                <div className="text-xs text-slate-500 mt-1">
                  {b.scope === "all" ? t("broadcast.allGuardians") : `${b.student_ids?.length ?? 0} students`} · {b.total} {t("broadcast.recipients")} · {b.email_sent} {t("broadcast.emails")} · {b.sms_sent} SMS
                </div>
              </div>
            ))}
          </div>
