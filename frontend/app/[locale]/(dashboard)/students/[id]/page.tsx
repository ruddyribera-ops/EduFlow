  ...
  return (
    <div className="p-6 space-y-5 max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-5">
          <h2 className="font-semibold text-slate-900 mb-3">{t("students.detail.guardians")} ({s.guardians?.length ?? 0})</h2>
              ...
              {s.guardians.map((g) => (
                <div key={g.id} className="text-sm border-b border-slate-100 last:border-0 pb-3 last:pb-0">
                  <div className="font-medium text-slate-900">
                    ...
                    {g.relationship_type && (
                      <span className="ml-2 text-xs font-normal text-slate-500">{t("students.detail.relationship", { relationship: g.relationship_type })}</span>
                    )}
                  </div>
